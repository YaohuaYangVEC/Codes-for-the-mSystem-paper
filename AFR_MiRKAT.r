...
afr_otu <- otu[as.character(afr_sample),]
afr_meta <- meta[as.character(afr_sample),]
meta <- afr_meta
otu <- afr_otu
...
afr <- meta$AFR
afr[is.na(afr)] <- 0.0
MiRKAT(y = afr, Ks = K.weighted, X = cova, out_type = "C", nperm=1000, method = "davies")
MiRKAT(y = afr, Ks = K.unweighted, X = cova, out_type = "C", nperm=1000, method = "davies")
MiRKAT(y = afr, Ks = K.BC, X = cova, out_type = "C", nperm=1000, method = "davies")          
